...
class Configuration:
    ...
    build_lock: AbstractContextManager[Any]
    _default_type_resolvers: dict[type[Any], TypeResolver[Any]]
    _settled_type_resolvers: dict[tuple[Any, Any], TypeResolver[Any]]

    def __init__(self) -> None:
        self.bindings = {}
        # The lock the injectors serialize their builds with. It lives
        # here and not in the injector, as the instances do: two
        # injectors sharing a configuration share the cached instances,
        # so they have to share the lock that guards building them
...
class Injector:
    ...
    def __init__(
        ...
        The lock comes from the configuration, so injectors sharing one
        serialize against each other, as they share the instances too.
        """
        self._configuration = configuration
        self._build_lock = configuration.build_lock if thread_safe else nullcontext()
